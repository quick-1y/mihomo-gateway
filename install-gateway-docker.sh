...
# Public configuration loaded from GitHub.
readonly CONFIG_URL="https://raw.githubusercontent.com/quick-1y/mihomo-gateway/main/gateway.conf"
readonly MIHOMO_TEMPLATE_URL="https://raw.githubusercontent.com/quick-1y/mihomo-gateway/main/mihomo.yaml.template"
readonly COMPOSE_TEMPLATE_URL="https://raw.githubusercontent.com/quick-1y/mihomo-gateway/main/compose.yaml.template"

# Built-in fallbacks are used only when the public gateway.conf cannot be downloaded.
LAN_DEFAULT="192.168.100.1"
DHCP_START_DEFAULT="192.168.100.100"
DHCP_END_DEFAULT="192.168.100.250"
DNS1_DEFAULT="1.1.1.1"
DNS2_DEFAULT="8.8.8.8"
TIMEZONE_DEFAULT="Europe/Moscow"
MIHOMO_PORT_DEFAULT="7890"
MIHOMO_API_PORT_DEFAULT="9090"
MIHOMO_IMAGE_DEFAULT="metacubex/mihomo:latest"
METACUBEXD_IMAGE_DEFAULT="ghcr.io/metacubex/metacubexd:latest"
...
MIHOMO_PORT="$MIHOMO_PORT_DEFAULT"
MIHOMO_API_PORT="$MIHOMO_API_PORT_DEFAULT"
MIHOMO_IMAGE="$MIHOMO_IMAGE_DEFAULT"
METACUBEXD_IMAGE="$METACUBEXD_IMAGE_DEFAULT"
NAT_ENABLED="1"
...
bootstrap_dependencies(){
    ...
    if (( ${#missing[@]} > 0 )); then
        # ca-certificates is required for HTTPS even when curl is already installed.
        apt_install ca-certificates curl gettext-base
    fi
}
...
# ----------------------------------------------------------------------------
load_public_config(){
    local tmp="/tmp/mihomo-gateway.conf.$$"
    if curl -fsSL --max-time 15 "$CONFIG_URL" -o "$tmp"; then
        # gateway.conf is maintained in the user's own repository.
        # It must contain assignments only; no commands should be placed there.
        # shellcheck disable=SC1090
        . "$tmp"
        rm -f "$tmp"
        success "Публичный gateway.conf загружен."
    else
        rm -f "$tmp"
        warn "Не удалось загрузить gateway.conf. Использую встроенные значения по умолчанию."
    fi

    LAN_DEFAULT="${LAN_DEFAULT:-192.168.100.1}"
    DHCP_START_DEFAULT="${DHCP_START_DEFAULT:-192.168.100.100}"
    DHCP_END_DEFAULT="${DHCP_END_DEFAULT:-192.168.100.250}"
    DNS1_DEFAULT="${DNS1_DEFAULT:-1.1.1.1}"
    DNS2_DEFAULT="${DNS2_DEFAULT:-8.8.8.8}"
    TIMEZONE_DEFAULT="${TIMEZONE_DEFAULT:-Europe/Moscow}"
    MIHOMO_PORT_DEFAULT="${MIHOMO_PORT_DEFAULT:-7890}"
    MIHOMO_API_PORT_DEFAULT="${MIHOMO_API_PORT_DEFAULT:-9090}"
    MIHOMO_IMAGE_DEFAULT="${MIHOMO_IMAGE_DEFAULT:-metacubex/mihomo:latest}"
    METACUBEXD_IMAGE_DEFAULT="${METACUBEXD_IMAGE_DEFAULT:-ghcr.io/metacubex/metacubexd:latest}"
    SUBSCRIPTION_URL_DEFAULT="${SUBSCRIPTION_URL_DEFAULT:-}"
...
METACUBEXD_IMAGE=${METACUBEXD_IMAGE}
LAN_IFACE=${LAN_IFACE}
WAN_IFACE=${WAN_IFACE}
LAN_MAC=${LAN_MAC}
WAN_MAC=${WAN_MAC}
...
load_config(){
    ...
    MIHOMO_PORT="${MIHOMO_PORT:-$MIHOMO_PORT_DEFAULT}"; MIHOMO_API_PORT="${MIHOMO_API_PORT:-$MIHOMO_API_PORT_DEFAULT}"
    MIHOMO_IMAGE="${MIHOMO_IMAGE:-$MIHOMO_IMAGE_DEFAULT}"; METACUBEXD_IMAGE="${METACUBEXD_IMAGE:-$METACUBEXD_IMAGE_DEFAULT}"
...
configure_network_first(){
    ...
    backup_once
    LAN_IP="$LAN_DEFAULT"; DHCP_START="$DHCP_START_DEFAULT"; DHCP_END="$DHCP_END_DEFAULT"; DNS1="$DNS1_DEFAULT"; DNS2="$DNS2_DEFAULT"; TIMEZONE="$TIMEZONE_DEFAULT"; MIHOMO_PORT="$MIHOMO_PORT_DEFAULT"; MIHOMO_API_PORT="$MIHOMO_API_PORT_DEFAULT"; MIHOMO_IMAGE="$MIHOMO_IMAGE_DEFAULT"; METACUBEXD_IMAGE="$METACUBEXD_IMAGE_DEFAULT"; NAT_ENABLED=1; SUBSCRIPTION_URL=""; CLASH_SECRET=""
    write_config
...
prompt_subscription_install(){
    ...
    if [[ -z "$SUBSCRIPTION_URL" && -n "${SUBSCRIPTION_URL_DEFAULT:-}" && ! -f "$MIHOMO_CONFIG" ]]; then
        SUBSCRIPTION_URL="$SUBSCRIPTION_URL_DEFAULT"
        info "Используется ссылка SUBSCRIPTION_URL_DEFAULT из gateway.conf."
        if curl -fsSL --max-time 10 -o /dev/null "$SUBSCRIPTION_URL"; then
            success "Ссылка из gateway.conf доступна."
            write_config
            return
        fi
        warn "Ссылка из gateway.conf не проверена."
...
render_mihomo_config(){
    ...
    local template tmp
    template=$(mktemp /tmp/mihomo.yaml.template.XXXXXX)
    if ! curl -fsSL --max-time 20 "$MIHOMO_TEMPLATE_URL" -o "$template"; then
        rm -f "$template"
        error "Не удалось загрузить mihomo.yaml.template."
        return 1
    fi

    LAN_NETWORK="${LAN_IP%.*}.0/24"
    export LAN_IP LAN_NETWORK CLASH_SECRET SUBSCRIPTION_URL MIHOMO_PORT MIHOMO_API_PORT
    if ! envsubst < "$template" > "$MIHOMO_CONFIG"; then
        rm -f "$template"
        error "Не удалось сформировать конфигурацию Mihomo."
        return 1
    fi
    rm -f "$template"
    chmod 600 "$MIHOMO_CONFIG"
    success "Конфигурация Mihomo сформирована из шаблона."
}

create_compose(){
    local template tmp
    template=$(mktemp /tmp/compose.yaml.template.XXXXXX)
    if ! curl -fsSL --max-time 20 "$COMPOSE_TEMPLATE_URL" -o "$template"; then
        rm -f "$template"
        error "Не удалось загрузить compose.yaml.template."
        return 1
    fi

    export TZ="$TIMEZONE" DEFAULT_BACKEND_URL="http://${LAN_IP}:${MIHOMO_API_PORT}" MIHOMO_IMAGE METACUBEXD_IMAGE
    if ! envsubst < "$template" > "$COMPOSE_FILE"; then
        rm -f "$template"
        error "Не удалось сформировать compose.yaml."
        return 1
    fi
    rm -f "$template"
    success "Создан compose.yaml из шаблона."
}
...
create_env(){
    ...
    cat > "$ENV_FILE" <<EOF2
TZ=${TIMEZONE}
DEFAULT_BACKEND_URL=http://${LAN_IP}:${MIHOMO_API_PORT}
EOF2
    chmod 600 "$ENV_FILE"
}
...
validate_config(){
    ...
    run_timed "Проверка конфигурации Mihomo" docker run --rm --network host --cap-add NET_ADMIN --device /dev/net/tun:/dev/net/tun -v "${MIHOMO_CONFIG_DIR}:/root/.config/mihomo" "$MIHOMO_IMAGE" -d /root/.config/mihomo -t
}
...
check_api(){
    ...
    curl -fsS --max-time 3 -H "Authorization: Bearer ${CLASH_SECRET}" "http://${LAN_IP}:${MIHOMO_API_PORT}/version" >/dev/null 2>&1
}
...
quick_status(){
    ...
    check_api && echo -e "  API :${MIHOMO_API_PORT}     $(service_dot ok)" || echo -e "  API :${MIHOMO_API_PORT}     $(service_dot FAIL)"
...
full_diagnostics(){
    ...
    echo "  Mihomo proxy:  ${LAN_IP}:${MIHOMO_PORT}"
    echo "  Mihomo API:    ${LAN_IP}:${MIHOMO_API_PORT}"
    ...
    echo "  API:        http://${LAN_IP}:${MIHOMO_API_PORT}"
    echo "  Proxy:      ${LAN_IP}:${MIHOMO_PORT}"
...
apply_generated(){
    ...
    write_nftables
    nft -c -f "$NFT_FILE"
    if ! run_timed "Применение nftables" nft -f "$NFT_FILE"; then return 1; fi
    if ! run_timed "Применение Netplan" apply_netplan_checked; then return 1; fi
    configure_dnsmasq
    render_mihomo_config
    create_env
    create_compose
...
subscription_menu(){
    while true; do
        ...
        case "$c" in
          ...
          2) [[ -n "$SUBSCRIPTION_URL" ]] && curl -fsSL --max-time 10 -o /dev/null "$SUBSCRIPTION_URL" && success "Подписка доступна." || error "Подписка недоступна." ;;
...
lan_menu(){
    ...
    echo; echo "Изменения: ${o} → ${LAN_IP}"; echo "DHCP gateway/DNS → ${LAN_IP}"; echo "Mihomo API → http://${LAN_IP}:${MIHOMO_API_PORT}"; echo
    warn "После применения текущая SSH-сессия может оборваться."
    if confirm "Применить новую LAN-конфигурацию?"; then
        if apply_generated; then success "LAN изменён. Переподключитесь к ssh user@${LAN_IP}"; else LAN_IP="$o"; write_config; warn "Изменение LAN не применено полностью."; fi
    else LAN_IP="$o"; fi
}
...
final_success(){
    ...
    echo -e "  Mihomo API: ${CYAN}${BOLD}http://${LAN_IP}:${MIHOMO_API_PORT}${NC}"
    echo -e "  Proxy: ${CYAN}${BOLD}${LAN_IP}:${MIHOMO_PORT}${NC}"
    echo; echo "  В MetaCubeXD используйте:"; echo "    Backend URL: http://${LAN_IP}:${MIHOMO_API_PORT}"; echo "    Secret: пароль, введённый при установке"; echo
}
...
self_test(){
    ...
    # Prevent the /etc/os-release readonly-variable bug from returning.
...
main_menu(){
    while true; do
        ...
        echo -e "  ${DIM}API:    http://${LAN_IP}:${MIHOMO_API_PORT}${NC}"
        echo
