...
for cand in \
  ...
  "C:/Users/admin/PycharmProjects/mihomo-gateway/.claude/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`C:/Users/admin/PycharmProjects/mihomo-gateway/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. An unresolved target exits in `get_installed_version` before this step.
...
global: `C:/Users/admin/PycharmProjects/mihomo-gateway/.claude/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
