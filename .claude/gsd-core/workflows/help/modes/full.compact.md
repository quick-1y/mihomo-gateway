...
**`/gsd-capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `C:/Users/admin/PycharmProjects/mihomo-gateway/.claude/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
