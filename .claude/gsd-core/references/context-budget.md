...
The keys live in `.claude/settings.json` (project) or `C:/Users/admin/PycharmProjects/mihomo-gateway/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @C:/Users/admin/PycharmProjects/mihomo-gateway/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
